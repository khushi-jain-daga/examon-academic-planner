...
const FAC_BY_NAME={
 'gaurav sir':'f-gaurav','shivam sir':'f-shivam','amrit sir':'f-amrit','anjna maam':'f-anjna','anjna ma\'am':'f-anjna','anjna ma’am':'f-anjna','anjana maam':'f-anjna','anjana ma\'am':'f-anjna','vivek sir':'f-vivek','rakhi maam':'f-rakhi','rakhi ma\'am':'f-rakhi','rakhi ma’am':'f-rakhi','joshit sir':'f-joshit'
};
const normName=n=>String(n||'').trim().toLowerCase().replace(/[’']/g,"'").replace(/\s+/g,' ');
const fid=n=>{const raw=normName(n);const compact=raw.replace(/[^a-z0-9]+/g,'');if(FAC_BY_NAME[raw])return FAC_BY_NAME[raw];if(FAC_BY_NAME[compact])return FAC_BY_NAME[compact];if(raw.includes('gaurav'))return'f-gaurav';if(raw.includes('shivam'))return'f-shivam';if(raw.includes('amrit'))return'f-amrit';if(raw.includes('anj'))return'f-anjna';if(raw.includes('vivek'))return'f-vivek';if(raw.includes('rakhi'))return'f-rakhi';if(raw.includes('joshit'))return'f-joshit';return''};
const ids=names=>String(names||'').split('&').map(s=>fid(s)).filter(Boolean);
const id=s=>'s-'+String(s).toLowerCase().replace(/&/g,'and').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
const iso=s=>{if(/^\d{4}-/.test(s))return s;const m={jan:'01',feb:'02',mar:'03',apr:'04',may:'05',jun:'06',jul:'07',aug:'08',sep:'09',oct:'10',nov:'11',dec:'12'};let a=String(s).trim().split(/\s+/);return `2026-${m[a[1].slice(0,3).toLowerCase()]}-${a[0].padStart(2,'0')}`};
const startTime=t=>{let x=String(t||'').split('-')[0].trim().replace(/\s+/g,' ');let m=x.match(/(\d{1,2})(?::(\d{2}))?\s*(AM|PM)?/i);if(!m)return x;let h=+m[1],min=m[2]||'00',ap=(m[3]||'').toUpperCase();if(ap==='PM'&&h<12)h+=12;if(ap==='AM'&&h===12)h=0;return String(h).padStart(2,'0')+':'+min};
...
function topic(m,i){if(m.subjectName==='Technical Question Practice'){const arr=['Thermo + RAC + IC Engine + Power Plant','Miscellaneous','SOM + Fluid Mechanics + Hydraulic Machine','Metal Cutting + Workshop Technology + Metrology','Metal Casting + Forming + Welding + Material Science'];return 'Technical Practice - '+i+' · '+arr[Math.min(4,Math.floor((i-1)/5))]+' - '+(((i-1)%5)+1)}return m.subjectName+' - '+i}
...
const state={version:2,faculty:FAC,subjects,plans:[...seeded,...customPlans],sourceAudit:{updatedAt:new Date().toISOString(),notes:'Seeded from source PDFs only. Rajdhani 18/449/276/173. QP 8/95/25/70. ONGC 5 modules/40. ISRO 12/325/268/57. Faculty mapping audited and fixed.'}};
